...
<table cellpadding="2" cellspacing="2" border="1">
    ...
    <% for (Item item: cart) { %>
        ...
        <tr>
            <td align="center"><a href="/cart/remove/<%= item.getProduct().getId() %>" onclick="return confirm('Are you sure?')">Remove</a></td>
            <td><%= item.getProduct().getId() %></td>
            <td><%= item.getProduct().getName() %></td>
            <td>IMAGE</td>
            <td><%= String.format("%.2f$", item.getProduct().getPrice()) %></td>
            <td><%= item.getQuantity() %></td>
            <td><%= String.format("%.2f$", item.getProduct().getPrice().multiply(new BigDecimal(item.getQuantity()))) %></td>
        </tr>
    <% } %>
    <tr>
